...
$context | add-member -Name catalogId -Value $catalog.id -MemberType NoteProperty

# check if groups already exist, create if not

$group = Get-AzADGroup -DisplayName $context.accessPackages[0].name
if(!($group)){
    $group = New-AzADGroup -DisplayName $context.accessPackages[0].name -SecurityEnabled -MailNickname $context.accessPackages[0].name
}

# grant group desired role on scope

$roleAssignment = Get-AzRoleAssignment -Scope $context.accessPackages[0].scope |Where-Object -FilterScript {$_.ObjectId -eq $group.Id -and $_.RoleDefinitionId -eq $context.accessPackages[0].role}
if(!($roleAssignment)){
    $roleAssignment = New-AzRoleAssignment -ObjectId $group.id -Scope $context.accessPackages[0].scope -RoleDefinitionId $context.accessPackages[0].role
}
...
$payload = @{
    "catalogId" = $context.catalogId
    "displayName" = $context.accessPackages[0].name
    "description" = $context.accessPackages[0].name
}
